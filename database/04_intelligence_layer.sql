...
-- 1️⃣ Learner Predictions
CREATE TABLE learner_predictions (
...
-- 2️⃣ STEM Score Intelligence
CREATE TABLE stem_scores (
...
-- 3️⃣ Risk Signals (Dropout / Low Performance / Ghost Patterns)
...
-- 4️⃣ National Analytics Summary
CREATE TABLE national_analytics_summary (
...
-- Performance Indexes
CREATE INDEX idx_predictions_learner_term 
ON learner_predictions(learner_id, term);

CREATE INDEX idx_stem_learner_term 
ON stem_scores(learner_id, term);

CREATE INDEX idx_risk_learner_term 
ON risk_signals(learner_id, term);

CREATE INDEX idx_national_county_term 
